...
A Pi session outside Herdr retains the separate Firstmate task-record fallback, clearly labeled as task records rather than open Herdr workspaces.
...
Pi's footer shows `telegram: ✓` in Pi's success color when the mirror is connected and enabled, `telegram: ✗` when it is connected and disabled, or `telegram: unavailable` when disconnected.
